...
class Node:
    ...
    def expand(self, problem):
        ...
        return [self.child_node(problem, action)
                for action in problem.takeActions(self.state)]

    def child_node(self, problem, action):
        "Fig. 3.10"
        next = problem.goal_test(self.state)
        return Node(next, self, action,
                    problem.stepCost(self.path_cost, self.state, action),problem.successorFn(self.state))
